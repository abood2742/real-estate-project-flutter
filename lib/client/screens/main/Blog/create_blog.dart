...
//                               ],
//                             )
//                           : const Text('لم يتم اختيار صور',style: TextStyle(color: Color.fromARGB(255, 22, 123, 191)),),
//                     ],
//                   ),
//                 ),
//               ),
...
//                   onPressed: _publish,
//                   label: const Text('نشر المدونة',style: TextStyle(color:Colors.white),),
...
class _CreateBlogPageState extends State<CreateBlogPage> {
  ...
  Future<void> _saveDraft() async {
    ...
    if (_titleController.text.isEmpty &&
        ...
        _imageBytes.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(localizations.translate('no_data_to_save'))),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(localizations.translate('draft_saved_success'))),
    );
  }
  ...
  void _publish() async {
    ...
    if (_titleController.text.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(localizations.translate('enter_title_content_images'))),
      );
      return;
    }
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        for (var file in _imageFiles) {
          photos.add(await MultipartFile.fromFile(file.path,
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(localizations.translate('blog_published_success'))),
        );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(localizations.translate('publish_error') + ': $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: primaryColor,
        title: Text(localizations.translate('create_blog'),style: TextStyle(color: Colors.white,fontFamily: 'Pacifico'),),
        centerTitle: true,
        actions: [
          TextButton.icon(
            onPressed: _saveDraft,
            icon: const Icon(Icons.save_alt_rounded, color: Colors.white),
            label: Text(
              localizations.translate('save_as_draft'),
      ...
      body: Directionality(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildSectionTitle(localizations.translate('blog_title')),
              const SizedBox(height: 8),
              _buildTextField(_titleController,
                  localizations.translate('enter_blog_title')),
              const SizedBox(height: 16),
              _buildSectionTitle(localizations.translate('blog_content')),
              const SizedBox(height: 8),
              _buildTextField(_contentController,
                  localizations.translate('enter_blog_content'),
                  maxLines: 6),
              const SizedBox(height: 20),
              _buildSectionTitle(localizations.translate('blog_images')),
              const SizedBox(height: 10),
              Card(
                elevation: 3,
                shape: RoundedRectangleBorder(
                    ...
                    children: [
                      ...
                      (_imageBytes.isNotEmpty || _imageFiles.isNotEmpty)
                          ...
                          : Text(localizations.translate('no_images_selected'),
                              style: TextStyle(
                                  color: Color.fromARGB(255, 22, 123, 191))),
                    ],
                  ),
                ),
              ),
              ...
              Center(
                child: ElevatedButton.icon(
                  onPressed: _publish,
                  label: Text(localizations.translate('publish_blog'),
                      style: TextStyle(color: Colors.white)),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: primaryColor,
                    padding: const EdgeInsets.symmetric(
