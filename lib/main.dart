...
void main() async {
  WidgetsFlutterBinding.ensureInitialized(); // لضمان تهيئة Flutter
  String? id = await PropertySystem().getUser();
  runApp(OverlaySupport.global(child: PropertySystem(id: id)));
}
...
class LocalizationProvider with ChangeNotifier {
  ...
  void setLocale(Locale locale) {
    if (!['ar', 'en'].contains(locale.languageCode)) {
      return; // تجاهل اللغات غير المدعومة
    }
    if (_locale != locale) {
      _locale = locale;
      notifyListeners();
      ...
      child: Consumer<LocalizationProvider>(
        builder: (context, provider, child) {
          return MaterialApp(
            ...
            // تعيين الصفحة الرئيسية دون Scaffold مباشرة
            home: InitialPage(),
            // إعادة بناء الواجهة مع دعم الاتجاه
            builder: (context, child) {
              return Directionality(
                textDirection: provider.locale.languageCode == 'ar'
                    ? TextDirection.rtl
